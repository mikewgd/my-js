(function () {	
	...
	*/
	ML.RC = {
		...
		*
		* @param {HTMLElement} findIn (optional) - an element to look for radio buttons and checkboxes. Good for when adding to the DOM.
		*/
		setup: function (findIn) {
			var self = this,
				inputs = findIn ? ML._$('input', findIn) : ML._$('input');

			ML.loop(inputs, function(input, i) {
				if (ML.hasClass(input, 'sys') || ML.hasClass(input, 'styled')) return;
