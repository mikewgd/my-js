...
ML.TabHandler = function() {
    var tags = ML._$('ul', document.body);

    for (var i = 0; i < tags.length; i++) {
        var attr = ML.El.data;

        if (attr(tags[i], 'tab-holder') !== null) {
            var dataTabHolder = attr(tags[i], 'tab-holder'),
                isClass = (dataTabHolder == '.') ? true : false,
                tabHolder = (isClass) ? ML.$C(dataTabHolder) : ML.$(dataTabHolder.replace('.', ''));

            new ML.Tabs(tabHolder, ML.El.clean(tags[i]));
        }
    }
};

ML.Tabs = function(holder, ul) {
    var tabs = ul.childNodeas
